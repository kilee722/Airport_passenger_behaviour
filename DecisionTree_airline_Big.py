...
#AriAirline Regroup (1. KE 2. OZ  (major airlines)  VS    Others )
airport['Airline'] = np.where(airport['Airline'] <=2, 1,0)
...
export_graphviz(clf, out_file=dot_data,  
                filled=True, rounded=True, special_characters=True, feature_names = X.columns,class_names=['KE','OZ','LCC','FA'])
...
plt.title('Depth of Decision Tree - Airline Model (KE, OZ versus LLC, Foreign Airline')
...
dot_data = StringIO()
export_graphviz(clf, out_file=dot_data,  
                filled=True, rounded=True, special_characters=True, feature_names = X.columns,class_names=['KE/OZ','Others'])
graph = pydotplus.graph_from_dot_data(dot_data.getvalue())  
